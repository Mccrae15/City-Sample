...
namespace CitySample.Automation
{
	public class CitySample_CheckAssetReferences : BuildCommand
	{
		...
		public void ExecuteBuildInner()
		{
			...
			if (ThisCLInt > 0)
			{
				...
				{
					Logger.LogInformation("Not writing PrevCLFile {PrevCLFilePath}. The current CL was not newer or -SkipPrevCLFileExport was specified", PrevCLFilePath);
				}
			}
		}
	...
	class CitySample_UpdateInCookCollection : BuildCommand
	{
		public override void ExecuteBuild()
		{
			Logger.LogInformation("************************* UpdateInCookCollection");
			...
			{
				Logger.LogInformation("UFS Path {UseUfsFilename} did not exist, using {DefaultUfsFilename} instead.", UseUfsFilename, DefaultUfsFilename);
...
#nullable disable
		...
		public void UpdateInCookAuditCollection(string CollectionName, string NotInCookCollectionName, string UfsFilename, bool SkipCheckin, bool WithNotInCookCollection)
		{
			...
			if (WithNotInCookCollection)
			{
				Logger.LogInformation("Discovering all game assets. This could take a while...");
				...
				Logger.LogInformation("Finished discovering all game assets! {Arg0}", UnusedGameAssets.Count.ToString());
				TotalAssetCount = UnusedGameAssets.Count;

				Logger.LogInformation("Discovering all game plugin assets. This could take a while...");
				...
				Logger.LogInformation("Finished discovering all game plugin assets! {Arg0}", (UnusedGameAssets.Count - TotalAssetCount).ToString());
				TotalAssetCount = UnusedGameAssets.Count;
			}
			...
			finally
			{
				...
				Logger.LogInformation("Total Used Assets = {Arg0}, Total Assets = {Arg1}", TotalUsedAssetCount.ToString(), TotalAssetCount.ToString());
			}
